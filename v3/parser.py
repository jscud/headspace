...
class Parser:
  ...
  def process_collection_member_access(self, parent_node):
    self.enter_method('process_collection_member_access')
    current_token = self.current_token()
    if current_token and current_token.matches('SYMBOL', '['):
      self.consume_current_token('processed opening [ in collection member access')
    else:
      print('member access must start with an opening [')
      sys.exit(1)

    collection_index = Node('COLLECTION_ACCESS_INDEX')
    self.process_whitespace(collection_index)
    self.process_rvalue(collection_index)
    current_token = self.current_token()
    if current_token and current_token.matches('SYMBOL', ']'):
      self.consume_current_token('processed closing ] in collection member access')
    else:
      print('the collection must end with a closing ]')
      sys.exit(1)
  ...
  def process_rvalue(self, parent_node):
    ...
    if next_token and next_token.token_type == 'SYMBOL' and next_token.content in INFIX_OPERATORS:
      ...
      if current_token and current_token.matches('SYMBOL', '('):
        ...
        self.process_collection_member_access(sub_node)
        sub_node.node_type = 'COLLECTION_MEMBER_ACCESS'
        parent_node.members.append(sub_node)
      else:

"""Takes a headspace project directory and converts to the target language.
...
SUPPORTED_LANGUAGES = ['c', 'python', 'go', 'javascript', 'java', 'dotnet', 'php', 'rust' , 'swift']
...
def convert_project(project_directory, target_language, print_log_messages=False):
  ...
  if not headspace_directory:
    sys.exit('The project directory did not contain a headspace source directory.')
    ...
    if source_location.is_file():
      with open(source_location, 'r') as source_file:
        ...
        for result_file in results_files:
          ...
          result_path = pathlib.Path(os.path.join(target_directory, result_file.file_path))
